...
public class IndexBuilderChunk(
    // Class dependencies
    ...
    WikipediaClientChunk wikipediaClient,
    ...
    public async Task BuildIndex(string[] pageTitles)
    {
        foreach (var title in pageTitles)
        {
            ...
            var chunks = sections.SelectMany(section =>
                        ...
                        .Take(25) // Limit to first 25 chunks per page to control costs
                        .ToImmutableList();

            var stringsToEmbed = chunks.Select(c => $"{c.Title} > {c.Section}\n\n{c.Content}"); // Prepare strings for embedding by adding some context
